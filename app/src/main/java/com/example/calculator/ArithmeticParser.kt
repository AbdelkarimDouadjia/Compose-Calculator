...
class Tokenizer(val input: String) {
    ...
    fun nextToken() {
        ...
        if ("+-×÷()sincostan√".contains(input[pos])) {
...
// To get the value of the factor and carry out unary operations, handle parenthesis, and handle scientific functions
fun parseFactor(tokenizer: Tokenizer): Double {
    ...
    if (tokenizer.token in listOf("sin(", "cos(", "tan(", "sqrt(")) {
        val functionName = tokenizer.token!!.dropLast(1)
        tokenizer.nextToken()
        val argument = parseExpression(tokenizer)
        if (tokenizer.token != ")") {
            throw IllegalArgumentException("Missing closing parenthesis for $functionName")
        }
        tokenizer.nextToken()
        return when (functionName) {
            ...
            "tan" -> tan(argument)
            "sqrt" -> sqrt(argument)
            else -> throw IllegalStateException("Invalid function: $functionName")
        }
    }
